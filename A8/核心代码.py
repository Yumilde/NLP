...
def tokenize_zh(text: str):
    # 中文 BLEU 的简化分词：按字符级比较，忽略空白
    return [ch for ch in text.strip() if not ch.isspace()]


def bleu_interpretation(score: float) -> str:
    if score < 0.1:
        return "分数较低，候选译文与参考译文在词汇和短语层面的重合较少。"
    if score < 0.3:
        return "分数中等偏低，译文传达了部分信息，但在表达或细节上仍有明显差异。"
    if score < 0.5:
        return "分数中等，候选译文与参考译文有较多重合，整体可理解但仍可优化。"
    if score < 0.7:
        return "分数较高，候选译文与参考译文接近，翻译质量较好。"
...
with tab1:
    ...
    st.write("输入英文句子，使用 Hugging Face 预训练模型完成英译中。")

    en_text = st.text_area(
        "请输入英文句子",
        placeholder="For example: Machine translation has evolved rapidly in recent years.",
        height=140,
        key="tab1_input",
    )
    ...
    if st.button("开始翻译", type="primary", use_container_width=True, key="tab1_btn"):
        if not en_text.strip():
            ...
            if err_msg:
                st.error(
                    "NMT 模型加载失败。请升级 torch 到 >=2.6 后重试。"
                    f"\n\n错误信息：{err_msg}"
                )
            else:
                st.success("翻译完成")
...
with tab2:
    ...
    st.write("输入英文句子后，系统将并排展示逐词直译结果与 NMT 译文。")

    compare_text = st.text_area(
        "请输入用于对比的英文句子",
        placeholder="For example: It rains cats and dogs.",
        height=140,
        key="tab2_input",
    )
...
with tab3:
    ...
    st.write("输入原文、参考译文与候选译文，系统自动计算 BLEU 分数。")
    ...
    if st.button("计算 BLEU 分数", type="primary", use_container_width=True, key="tab3_bleu_btn"):
        if not reference_zh.strip() or not candidate_zh.strip():
            ...
            if not ref_tokens or not cand_tokens:
                st.error("输入内容过短或仅为空白，无法计算 BLEU。")
            else:
                ...
                st.info(
                    "BLEU 取值范围通常在 0 到 1 之间，数值越高表示候选译文与参考译文在 n-gram 重合上越接近。"
                )
